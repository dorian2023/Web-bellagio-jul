/**
 * @file stores.js
 * @description Official showroom and store data for Muebles Bellagio in Caracas, Venezuela.
 */

export const STORES_DATA = [
  {
    id: 'tienda-comercio',
    name: 'Bellagio Avenida Comercio',
    badge: 'Sede Principal',
    videoUrl: '/videos/tienda-principal.mp4',
    posterUrl: '/images/hero-poster.jpg',
    address: 'Av. Comercio, Caracas, Distrito Capital, Venezuela.',
    landmark: 'Edf. Comercial Bellagio, Planta Baja',
    phone: '+58 212-0000000',
    mobile: '+58 414-1536516',
    whatsapp: '584141536516',
    schedule: 'Lunes a Sábado: 9:00 AM - 6:30 PM',
    mapsUrl: 'https://www.google.com/maps/search/?api=1&query=Av+Comercio+Caracas+Venezuela',
    features: ['Showroom de Exhibición', 'Estacionamiento Privado', 'Asesoría de Interiorismo In-Situ']
  },
  {
    id: 'tienda-mobili',
    name: 'Muebles Bellagio Mobili',
    badge: 'Showroom Mobili',
    videoUrl: '/videos/tienda-mobili.mp4',
    posterUrl: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=800&q=80',
    address: 'Av. Comercio de Bella Vista, vía La Yaguara, C.C. Davinci, Caracas.',
    landmark: 'Centro Comercial Davinci, Nivel Principal',
    phone: '+58 212-0000001',
    mobile: '+58 414-1536516',
    whatsapp: '584141536516',
    schedule: 'Lunes a Sábado: 9:30 AM - 6:00 PM',
    mapsUrl: 'https://www.google.com/maps/search/?api=1&query=Centro+Comercial+Davinci+La+Yaguara+Caracas',
    features: ['Colecciones Importadas', 'Zona de Tapicería Premium', 'Atención Personalizada']
  },
  {
    id: 'tienda-casamall',
    name: 'Bellagio Casa Mall',
    badge: 'Showroom Design',
    videoUrl: '/videos/tienda-casamall.mp4',
    posterUrl: 'https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=800&q=80',
    address: 'Av. Principal de Los Naranjos, C.C. Casa Mall, Nivel Galería, local G20, Urb. El Cafetal, Caracas.',
    landmark: 'C.C. Casa Mall - Nivel Galería, Local G20',
    phone: '+58 212-0000002',
    mobile: '+58 414-1536516',
    whatsapp: '584141536516',
    schedule: 'Lunes a Domingo: 10:00 AM - 7:00 PM',
    mapsUrl: 'https://www.google.com/maps/search/?api=1&query=Casa+Mall+Los+Naranjos+Caracas',
    features: ['Diseño de Vanguardia', 'Abierto los Domingos', 'Servicio de Proyectos 3D']
  }
];
